  ...
  const transaction = db.transaction(() => {
    ...
    if (restoreModules.includes('grades') && data.grading_weights) {
      ...
      for (const w of data.grading_weights) {
        insert.run(w.id, w.database_id, w.midterm_collective, w.final_initial, w.final_final, w.midterm_exam, w.final_exam,
          w.pass_midterm, w.pass_initial, w.pass_final, w.pass_overall, w.freeze_final, w.custom_formula ?? null);
      }
    }
    ...
    if (restoreModules.includes('quizzes') && data.quizzes) {
      ...
      const insertQ = db.prepare(`
        INSERT INTO quizzes (id, title, type, subject, grade_level, period, passing_score, attempts_allowed,
      ...
      for (const q of data.quizzes) {
        insertQ.run(q.id, q.title, q.type, q.subject, q.grade_level, q.period ?? null,
          q.passing_score, q.attempts_allowed, q.time_limit, q.retake_allowed,
          q.randomize_questions, q.randomize_choices, q.status,
      ...
      if (data.questions) {
        const insertQn = db.prepare(`
          INSERT INTO questions (id, quiz_id, question_text, question_type,
            ...
            choice_a_media_url, choice_b_media_url, choice_c_media_url, choice_d_media_url)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
        for (const q of data.questions) {
          insertQn.run(
            ...
            q.media_type ?? null, q.media_url ?? null,
            q.choice_a_media_url ?? null, q.choice_b_media_url ?? null,
            q.choice_c_media_url ?? null, q.choice_d_media_url ?? null
          );
        }
      }
