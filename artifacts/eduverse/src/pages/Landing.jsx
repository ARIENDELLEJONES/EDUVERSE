...
export default function Landing({ onLogin, showToast }) {
  ...
  useEffect(() => {
    if (view === 'login' && role === 'student' && mode === 'quiz') {
      api.get('/students/assigned-grade-levels?mode=B').then((res) => {
        if (res.success && res.gradeLevels?.length) setGradeLevels(res.gradeLevels);
      });
    }
  }, [view, role, mode]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        user.mode = mode;
        // Store grade student password so GradeStudentPortal can reload
        // data after session expiry or manual refresh without losing grades.
        if ((mode === 'grades' || mode === 'A') && role === 'student') {
          localStorage.setItem('eduverse_student_pw', password);
        }
